...
import { devSignInWithGoogle, isDevelopment } from "../utils/devAuth";
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      try {
        ...
        // If any Firebase error and we're in development, use mock auth as fallback
        if (isDevelopment()) {
          console.warn("⚠️ Firebase authentication failed. Using development auth fallback...");
          response = await devSignInWithGoogle();
          usedDevAuth = true;
          console.log("✅ Development Sign-In Used:", response.user.email);
        } else {
      ...
      if (!response || !response.user) {
        throw new Error("No user data from authentication");
      }
      ...
      console.log("📤 Sending to server:", { name, email, devAuth: usedDevAuth });
